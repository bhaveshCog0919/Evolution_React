...
class Renewal_Not_Sent_Report extends Component {
    ...
    Renewal_Not_Sent_Report() {
        ...
        api.post('api/Renewal_Not_Sent_Report', data).then(res => {
            if (res.success) {
                var formattedData = [];
                this.setState({
                    selectedPage: 0
            ...
            <div>
                ...
                <div className="table-custom">
                    <DataTable ref={(el) => this.dt = el} value={this.state.arr} first={this.state.selectedPage} onPage={(e) => this.getPageString(e)}
                        paginator={true} rows={this.state.rowsPerPage} header={header} totalRecords={this.state.arr.length} exportFilename="Renewal Not Sent Report"
...
export default withTranslation()(Renewal_Not_Sent_Report);
